...
def infer_date_from_filename(name):
    ...
    # Wed_Mar_04_2026, Sun-Mar-01-2026, Mar_04_2026
    m = re.search(
    ...
    if m:
        ...
        if month:
            return pd.Timestamp(year=int(m.group(3)), month=month, day=int(m.group(2)))

    # 2026-03-04 / 2026_03_04
    m = re.search(r"(\\d{4})[_\\- ](\\d{1,2})[_\\- ](\\d{1,2})", text)
    if m:
        return pd.Timestamp(year=int(m.group(1)), month=int(m.group(2)), day=int(m.group(3)))

    # 04-03-2026 / 04_03_2026 fallback, interpreted dd-mm-yyyy
...
if uploaded_db is not None:
    try:
        ...
        st.sidebar.error(f"Kunde inte läsa trenddatabasen: {e}")
