...
def infer_opponent_from_text(text):
    """
    Best-effort opponent extraction.
    If a real Opponent/Motståndare column exists, that is used instead.
    For filenames/labels, this returns a compact readable label.
    """
    value = str(text or "").strip()
    value = re.sub(r"\.(csv|xlsx)$", "", value, flags=re.IGNORECASE)
    value = value.replace("_", " ").replace("-", " ")
    value = re.sub(r"\s+", " ", value).strip()

    # Remove common report/date words
    value = re.sub(r"\b(Mon|Tue|Wed|Thu|Fri|Sat|Sun)\b", "", value, flags=re.IGNORECASE)
    value = re.sub(r"\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\b\s+\d{1,2}\s+\d{4}", "", value, flags=re.IGNORECASE)
    value = re.sub(r"\b\d{4}\s+\d{1,2}\s+\d{1,2}\b", "", value)
    value = re.sub(r"\bPost Game Report\b", "", value, flags=re.IGNORECASE)
    value = re.sub(r"\bAll Periods\b", "", value, flags=re.IGNORECASE)
    value = re.sub(r"\bReport\b", "", value, flags=re.IGNORECASE)
    value = re.sub(r"\s+", " ", value).strip()

    # If nothing useful remains, return original shortened label
    if not value:
        value = str(text or "").strip()
...
def add_trend_axis_labels(df):
    """
    Adds Game Number and Trend X Label:
    1 | 1/3 | IFB
    2 | 4/3 | BIK
    etc.
    """
    ...
    if "Match Date" in out.columns:
        out["Match Date"] = pd.to_datetime(out["Match Date"], errors="coerce")
        out = out.sort_values(["Entity", "Match Date", "Match Label"], na_position="last").reset_index(drop=True)
    else:
        out = out.reset_index(drop=True)

    if "Entity" in out.columns:
        out["Game Number"] = out.groupby("Entity").cumcount() + 1
    else:
        out["Game Number"] = range(1, len(out) + 1)

    opponent_col = next((c for c in ["Opponent", "Motståndare", "Opp"] if c in out.columns), None)

    labels = []
    for _, row in out.iterrows():
        game_no = row.get("Game Number", "")
        date = row.get("Match Date", pd.NaT)

        if pd.notna(date):
            date_label = f"{pd.to_datetime(date).day}/{pd.to_datetime(date).month}"
        else:
            date_label = ""

        if opponent_col:
            opponent = str(row.get(opponent_col, "")).strip()
        else:
            opponent = infer_opponent_from_text(row.get("Match Label", row.get("Source File", "")))
...
with tab_trend:
    ...
    if not selected_metrics:
        ...
        for entity in trend_df["Entity"].dropna().astype(str).unique():
            ...
            x_values = entity_df["Trend X Label"] if "Trend X Label" in entity_df.columns else entity_df.index

            hover_data = []
            for _, hover_row in entity_df.iterrows():
                match_date = hover_row.get("Match Date", pd.NaT)
                date_text = pd.to_datetime(match_date).strftime("%Y-%m-%d") if pd.notna(match_date) else ""
                hover_data.append(
                    [
                        date_text,
                        hover_row.get("Match Label", ""),
                        hover_row.get("Game Number", ""),
                    ]
                )
            ...
            for metric in selected_metrics:
                fig.add_trace(
                    go.Scatter(
                        ...
                        hovertemplate="Match %{customdata[2]}<br>%{customdata[0]}<br>%{customdata[1]}<br>%{y}<extra></extra>",
                    )
                )

        fig.update_layout(
            height=700,
            xaxis_title="Matchnummer | datum | motståndare",
            yaxis_title="Råvärde",
        ...
        display_cols = [c for c in ["Game Number", "Trend X Label", "Match Date", "Entity", "Match Label"] if c in trend_df.columns]
        st.dataframe(trend_df[display_cols + selected_metrics], use_container_width=True)
