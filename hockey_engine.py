...
def build_metric_catalog_for_mode(taxonomy_df, analysis_mode, data_columns):
    ...
    for _, row in taxonomy_df.iterrows():
        ...
        if analysis_mode != "__ALL__" and modes and analysis_mode not in modes:
            continue
    ...
    if "Player" in df.columns:
        ...
        if players:
            selected_players = st.sidebar.multiselect(
                ...
                options=players,
                default=players,
                key=make_widget_key("players_filter", analysis_mode),
            )
    ...
    if "Season" in df.columns:
        ...
        if seasons:
            selected_seasons = st.sidebar.multiselect(
                ...
                options=seasons,
                default=seasons,
                key=make_widget_key("season_filter", analysis_mode),
            )
