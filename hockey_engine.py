...
def render_analysis_page(analysis_mode):
    ...
    if selected_metrics:
        with st.sidebar.expander(f"Valda metrics ({len(selected_metrics)})", expanded=False):
    ...
    selected_rows = selection_scope_df[selection_scope_df["Metric"].isin(selected_metrics)].copy()
