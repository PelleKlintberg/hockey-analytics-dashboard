...
BLOCKS_BY_MODE = {
    ...
    "Spelare": ["Overall", "Generell", "Offensivt", "Defensivt", "WOI", "Alla hittade metrics", "Saknade metrics", "Alla taxonomy metrics"],
    "Målvakt": ["Overall", "Målvakt", "Alla hittade metrics", "Saknade metrics", "Alla taxonomy metrics"],
}
...
GOALIE_OVERALL_PRESET_METRICS = [
    "SVS%", "SVS", "Save %", "Save%", "Saves %", "Save Percentage",
    "Räddnings%", "Räddnings %", "GA", "Goals Against", "Goals Against Average",
    "GAA", "xGA", "Expected Goals Against", "xG Against",
    "Retur", "Rebound", "Rebounds", "Rebound control", "Rebound Control",
    "Traffic", "Trafic", "Screens", "Screen", "Screened shots",
    "Shots Against", "Shots on Goal Against", "Shots On Net Against",
    "Goalie SH Save%", "Shootout saves", "xG per shot saved"
]
...
def should_override_taxonomy_block(block, metric):
    name = str(metric).lower()
    fb_block, fb_drill = fallback_block(metric)

    if block in ["", None, "Overall", "Generell", "Taxonomy / ej mappad"]:
        return True

    if specialteam_state(metric) is not None:
        return block != "Specialteam"
    if "woi" in name:
        return block != "WOI"
    if contains_any(name, ["goalie", "save", "saves", "gaa", "svs", "räddning", "shootout"]):
        return block != "Målvakt"
    if contains_any(name, ["faceoff", "face-off", "faceoffs"]):
        return block != "Faceoff"
    if " nz " in f" {name} " or name.startswith("nz"):
        return block != "NZ"
    if fb_block in ["Defensivt", "Offensivt", "Playmaking"] and block == "Overall":
        return True
    return False


def corrected_block_drill(raw_block, raw_drill, metric):
    fallback_b, fallback_d = fallback_block(metric)
    block = raw_block or fallback_b
    drill = raw_drill or fallback_d

    if should_override_taxonomy_block(block, metric):
        block, drill = fallback_b, fallback_d

    if block == "Defensivt" and drill in ["Carry out", "Dump out / dump-in recovery exit", "Pass out", "Outlet"]:
...
def build_taxonomy_browser(taxonomy_rows, all_metric_names):
    ...
    for r in taxonomy_rows:
        ...
        if not metric:
            continue
        raw_block = r.get("block") or r.get("main")
        raw_drill = r.get("drilldown") or r.get("sub")
        block, drill = corrected_block_drill(raw_block, raw_drill, metric)
...
def render_analysis_page(analysis_mode):
    ...
    if selected_block == "Alla hittade metrics":
        ...
        if analysis_mode == "Målvakt":
            goalie_extra = browser_df[
                (browser_df["Block"] == "Målvakt")
                | (browser_df["Metric"].astype(str).str.lower().str.contains("save|svs|räddning|gaa|goalie|rebound|screen|traffic|trafic|shot|against", na=False))
            ].copy()
            block_df = pd.concat([block_df, goalie_extra], ignore_index=True).drop_duplicates(subset=["Metric"])
    ...
    if block_df.empty:
        tmp = browser_df.copy()
        tmp["_fallback_block"] = tmp["Metric"].map(lambda m: fallback_block(m)[0])
        block_df = tmp[tmp["_fallback_block"] == selected_block].drop(columns=["_fallback_block"], errors="ignore").copy()
